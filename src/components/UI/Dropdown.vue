  ...
  <div :class="{ 'w-full': props.width === 'full', 'w-full xs:w-[415px]': props.width != 'full' }">
    <div class="flex justify-between">
      <label :for="props.label"
        :class="{ 'font-bold': props.bold === true, 'font-normal': props.bold !== true, 'invisible': props.displayLabel === false }">{{
          props.label }}</label>
      <p class="text-secondary-red underline cursor-pointer text-[0.9rem]" @click="deleteFilter"
        :class="{ 'hidden': props.display === false }">Verwijder</p>
    </div>

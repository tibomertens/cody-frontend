    ...
    <div class="rounded-[5px] flex-1"
        ...
        <div class="flex gap-[32px] justify-center items-center py-[32px]" :class="{ 'pl-[32px]': props.suggestion }">
            ...
            <div>
                ...
                <p v-if="props.budget === true" class="font-light text-[0.9em]"
                    :class="{ 'text-secondary-red !font-bold': text < 0 }">{{ formatFinancialNumber(props.text) }}</p>
                <p v-else class="font-light text-[0.9em]" :class="{ 'text-secondary-red font-bold': text < 0 }">{{ text }}
                </p>
            </div>
        </div>
    </div>

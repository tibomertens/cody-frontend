...
const changePassword = async () => {
    ...
    if(!updatedEmail.value || !updatedNewPassword.value || !updatedConfirmPassword.value) {
      hasError.value = true;
  ...
  let result = await updatePassword(updatedEmail.value, updatedNewPassword.value);
    if (result.success) {
      router.push("/login");
    } else {
      // Handle authentication error
      hasError.value = true;
      error.value = "Er liep iets fout bij het veranderen van je wachtwoord";
    }
...
<template>
  <form>
    ...
    <Input :label="'Nieuw wachtwoord'" :type="'password'" @input-change="updateNewPassword" :error="hasError" :forget="true"></Input>
    <Input :label="'Bevestig wachtwoord'" :type="'password'" @input-change="updateConfirmPassword" :error="hasError" :forget="true"></Input>
    <div v-if="error" class="text-secondary-red">{{ error }}</div>
    <div class="mt-8 mb-4">
      <Btn :name="'Wachtwoord veranderen'" @click="changePassword" :width="'full'" />
    </div>
  </form>
</template>

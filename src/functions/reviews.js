...
export const getReviewById = async (reviewId) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/reviews/review/${reviewId}`);
    let data = await response.json();
    return data.data;
  } catch (error) {
    console.error(error);
...
export const updateReview = async (promotorId, userId, reviewId, selectedStars, updatedTitle, updatedExperience) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/reviews/${reviewId}`, {
      method: "PUT",
      headers: {
